...
public static class EditorFileHelper
{
    ...
    public static void CreateFile(string fileName)
    {
        if (!IsFileExists(fileName))
        {
            ...
            stream.Close ();
#else
            FileStream fileStream = File.Create(GetFullPath(fileName));
            fileStream.Close();
#endif
    ...
    /// <summary>
    /// 根据指定地址获取该地址下所有文件列表(没有后缀)
    /// </summary>
    /// <param name="path"></param>
    /// <returns></returns>
    public static List<string> GetAssetListNoSuffixByPath(string path)
    {
        List<string> list = new List<string>();

        if (IsFolderExists(path))
        {
            DirectoryInfo direction = new DirectoryInfo(GetFullPath(path));
    ...
    /// <summary>
    /// 根据指定地址获取该地址下所有文件列表(包含后缀)
    /// </summary>
    /// <param name="path"></param>
    /// <returns></returns>
    public static List<string> GetAssetListWithSuffixByPath(string path)
    {
        List<string> list = new List<string>();
    ...
    /// <summary>
    /// 通过过滤条件获取指定文件夹下所有指定类型文件路径
    /// </summary>
    /// <returns></returns>
    public static List<string> GetAllPathUnderFolderWithFilter(string filter, string folderPath)
    {
        ...
        {
            var guides = AssetDatabase.FindAssets(filter, new[] {folder});
            var assetPaths = new string[guides.Length];
            int i;
            int iMax;
            for (i = 0, iMax = assetPaths.Length; i < iMax; ++i)
            {
                pathList.Add(AssetDatabase.GUIDToAssetPath(guides[i]));
            }
        }
    ...
    /// <summary>
    /// 通过过滤条件获取指定文件夹下所有指定类型文件Guid
    /// </summary>
    /// <returns></returns>
    public static List<string> GetAllGuidUnderFolderWithFilter(string filter, string folderPath)
    {
        var guidList = new List<string>();
        var subFolders = Directory.GetDirectories(folderPath).ToList();
        subFolders.Insert(0, folderPath);
        foreach (var guides in subFolders.Select(folder => AssetDatabase.FindAssets(filter, new[] {folder})))
        {
            guidList.AddRange(guides);
        }
    ...
    /// <summary>
    /// 通过过滤条件获取指定文件夹下所有指定类型文件Guid和路径
    /// </summary>
    /// <returns></returns>
    public static Dictionary<string, string> GetAllPathGuidUnderFolderWithFilter(string filter, string folderPath)
    {
        var dic = new Dictionary<string, string>();

        var folders = Directory.GetDirectories("Assets").ToList();
        folders.Insert(0, folderPath);
        foreach (var folder in folders)
        {
            var guides = AssetDatabase.FindAssets(filter, new[] {folder});
            ...
            for (i = 0, iMax = assetPaths.Length; i < iMax; ++i)
            {
                dic.Add(guides[i], AssetDatabase.GUIDToAssetPath(guides[i]));
            }
        }
